          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isCompleted && (
                <div className="flex items-center gap-2 ml-auto">
                  <span className="text-xs text-slate-600 font-medium">Attiva Bonus</span>
                  <button
                    onClick={onToggle}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 ${
                      isActive ? 'bg-gradient-to-r from-green-500 to-emerald-600' : 'bg-gray-300'
                    }`}
                    role="switch"
            ...
            {isCompleted && isActive && (
              <p className="text-xs text-emerald-600 font-bold mt-1">
                💰 +€{yearlyBonus.toLocaleString('it-IT')}/anno attivo
              </p>
            )}
          </div>
        </div>
...
const BonusProgress: React.FC<BonusProgressProps> = ({ totalContracts, onBonusChange }) => {
  ...
  const milestones = [
    { target: 600, amount: "+300€/mese", icon: <MedalIcon />, levelNum: 600, managerTitle: "Pro Manager", yearlyBonus: 3600 },
    { target: 1500, amount: "+1000€/mese", icon: <TrophyIcon />, levelNum: 1500, managerTitle: "Regional Manager", yearlyBonus: 12000 },
    { target: 5000, amount: "+3000€/mese", icon: <CrownIcon />, levelNum: 5000, managerTitle: "National Manager", yearlyBonus: 36000 },
  ];
    ...
    setActiveBonuses(prev => {
      ...
      milestones.forEach(milestone => {
        if (newState[milestone.levelNum] && totalContracts >= milestone.target) {
          highestBonus = Math.max(highestBonus, milestone.yearlyBonus);
        }
      });
        ...
        <div className="space-y-4">
            {milestones.map((milestone) => (
                <BonusCard
                    ...
                    t={t}
                    yearlyBonus={milestone.yearlyBonus}
